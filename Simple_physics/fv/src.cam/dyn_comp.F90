...
use commap,             only: clat, clon, clat_staggered, londeg_st ! added from 2.2
use spmd_dyn,           only: spmd_readnl

use inic_analytic,      only: analytic_ic_active, analytic_ic_set_ic ! added from 2.2
use dyn_tests_utils,    only: vc_moist_pressure ! added from 2.2
...
subroutine read_inidat(dyn_in)
  !use inic_analytic,   only: analytic_ic_active, analytic_ic_set_ic ! now set at top of file
...
!  type(file_desc_t), pointer      :: fh_topo
 ...
 ! fh_topo => topo_file_get_id()
  ...
  if (analytic_ic_active()) then
    ...
    call analytic_ic_set_ic(vc_moist_pressure, clat(jfirstxy:jlastxy),        &
         ...
         Q=dyn_in%tracer(:,:,:,1:ntotq), PHIS_IN=dyn_in%phis, m_cnst=m_cnst)  ! added
 ...
 !   fieldname = 'PHIS' ! now in set_phis
 !   readvar   = .false.
 !   if (.not. associated(fh_topo)) then
 !     dyn_in%phis(:,:) = 0._r8
 !   else
 ...
 ! call process_inidat(grid, dyn_in, 'PHIS', fh_ini=fh_ini) ! now in set_phis
...
subroutine set_phis(dyn_in)
   ...
   if (associated(fh_topo)) then    
      ...
      call infld(fieldname, fh_topo, 'lon', 'lat', ifirstxy, ilastxy, jfirstxy, jlastxy, &
         dyn_in%phis, readvar, gridname='fv_centers')
      if (.not. readvar) call endrun(sub//': ERROR: PHIS not found')
      write(iulog,*)'Read PHIS from topo'
...
subroutine process_inidat(grid, dyn_in, fieldname, m_cnst, fh_ini)
   ...
   type(file_desc_t),   optional, intent(inout) :: fh_ini      ! added optional from 2.2
   ...
   case ('CONSTS')
      ...
      if (readvar) then
         ...
         if (.not. present(fh_ini)) then ! added check from 2.2
