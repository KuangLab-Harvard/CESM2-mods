...
!======================================================================= 
  ...
  !TREF-READ, LFRAC-READ
  !subroutine held_suarez_1994(pcols, ncol, clat, pmid, &
  !     u, v, t, du, dv, s)
   subroutine held_suarez_1994(pcols, ncol, clat, pmid, &
       u, v, t, du, dv, s, treflchnk, lfracchnk)
  !END TREF-READ, LFRAC-READ
    ...
    !TREF-READ
    real(r8), optional  :: treflchnk(pcols,pver) !Tref from file
    !END TREF-READ

    !LFRAC-READ
    real(r8), optional  :: lfracchnk(pcols) !Tref from file
    !END LFRAC-READ
    ...
    do k = 1, pver
      if (pref_mid_norm(k) > sigmab) then
        ...
        do i = 1, ncol
          !LFRAC-READ -- adjust frictional coefficient by land mask
          if (present(lfracchnk)) then
            du(i,k) = -kv*u(i,k)*(2.0_r8/3.0_r8 + 4.0_r8/3.0_r8*lfracchnk(i))
            dv(i,k) = -kv*v(i,k)*(2.0_r8/3.0_r8 + 4.0_r8/3.0_r8*lfracchnk(i))
          !END LFRAC-READ
          else
          du(i,k) = -kv*u(i,k)
          dv(i,k) = -kv*v(i,k)
