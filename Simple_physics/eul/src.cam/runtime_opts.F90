...
!TREF-READ
! Modified to read in namelist parameters for reading in relaxation 
! temperature profile from file
! 
! Isla Simpson 24th Feb 2017
! Updated for CESM2 release, Isla Simpson, 31st May 2018
!END TREF-READ
...
subroutine read_namelist(nlfilename, single_column, scmlat, scmlon)
   ...
   !TREF-READ
   use held_suarez_cam,     only: trefread_readnl
   !END TREF-READ
   !LFRAC-READ
   use held_suarez_cam,     only: lfracread_readnl
   !END LFRAC-READ
...
!   call pbuf_readnl(nlfilename)
   ...
   ! TREF-READ
   call trefread_readnl(nlfilename)
   ! END TREF-READ
   ! LFRAC-READ
   call lfracread_readnl(nlfilename)
   ! END LFRAC-READ
