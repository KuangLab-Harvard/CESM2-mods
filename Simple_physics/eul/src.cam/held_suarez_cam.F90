...
module held_suarez_cam
 ...
 ! use ppgrid,       only: pcols, pver
  ...
  !LFRAC-READ
  character(len=256) :: lfracfile
  logical:: lfracfromfile
  real(r8),allocatable::lfrac(:,:) ! relaxation temperature read from file
  !LFRAC-READ
...
!======================================================================= 
  ...
  subroutine held_suarez_init(pbuf2d)
    ...
    !LFRAC-READ
     allocate(lfrac(pcols,begchunk:endchunk),stat=istat)
     call alloc_err(istat,'UPDATE_LFRAC_EUL','lfrac',pcols*(endchunk-begchunk))
     lfrac(:pcols,begchunk:endchunk)=0._r8
     if (lfracfromfile) then
      call update_lfrac(lfracfile)
     endif
    !END LFRAC-READ
  ...
  subroutine held_suarez_tend(state, ptend, ztodt)
    ...
    !LFRAC-READ
    real (r8)                          :: lfracchnk(pcols)    ! Land frac of current chunk
    ...
    if (treffromfile) then
      if (lfracfromfile) then
        call held_suarez_1994(pcols, ncol, clat, pmid, &
          ...
          treflchnk=treflchnk, lfracchnk=lfracchnk)
      else
        call held_suarez_1994(pcols, ncol, clat, pmid, &
          state%u, state%v, state%t, ptend%u, ptend%v, ptend%s, &
          treflchnk=treflchnk)
...
!LFRAC-READ
  subroutine lfracread_readnl(nlfile)
    ! -----------------------------------------------------------------------
    ! Read in namelist parameters lfracfromfile and lfracfile
    ! lfracfile = controls whether the land / sea mask
    ! is the default (lfracfile=.False.) or is read in from
    ! file (lfracfile=.True.) 
    ! Sarah Weidman (March 2024)
    ! -----------------------------------------------------------------------
        use namelist_utils  ,only:find_group_name
        use units           ,only:getunit,freeunit
        character(len=*),intent(in)::nlfile
        integer :: unitn, ierr
        character(len=*), parameter :: sub = 'lfracread_readnl'
    
    
        namelist /lfracread_nl/ lfracfromfile,lfracfile 
    
        !Set default namelist values
        lfracfromfile=.False.
        lfracfile='randomfile'
    
        !Read in namelist values
        if (masterproc) then
         unitn = getunit()
         open(unitn,file=trim(nlfile),status='old')
         call find_group_name(unitn,'lfracread_nl',status=ierr)
         if (ierr.eq.0) then
           read(unitn,lfracread_nl,iostat=ierr)
           if (ierr.ne.0) then
             call endrun('lfracread_readnl:: ERROR reading namelist')
           endif
         endif
         close(unitn)
         call freeunit(unitn)
       ...
       call mpi_bcast(lfracfromfile   , 1, mpi_logical, mstrid, mpicom, ierr)
       if (ierr /= 0) call endrun(sub//": FATAL: mpi_bcast: held_suarez_1994")
       call mpi_bcast(lfracfile       , len(lfracfile), mpi_character, mstrid, mpicom, ierr)
       if (ierr /= 0) call endrun(sub//": FATAL: mpi_bcast: held_suarez_1994")
    ...
    !END LFRAC-READ 
      ...
      !LFRAC-READ
      subroutine update_lfrac(lfracfilein)
      ! --------------------------------
      ! Read in land sea mask from a netcdf file 
      ! Sarah Weidman March 2024
      ! --------------------------------
       use cam_grid_support, only: cam_grid_get_dim_names, cam_grid_id
       use pio, only: pio_nowrite, file_desc_t, pio_closefile
       ...
       character(len=*), intent(in) :: lfracfilein
       character(len=8) :: dim1name, dim2name
       integer :: grid_id
       type(file_desc_t), pointer :: ncidlfrac
       character(len=256) :: lfracpath
       logical :: found=.false.
       ...
       allocate(ncidlfrac)
       call getfil(lfracfilein, lfracpath)
       call cam_pio_openfile(ncidlfrac, trim(lfracpath), PIO_NOWRITE)
       call infld('LANDFRAC',ncidlfrac,dim1name,dim2name,1,pcols,  &
         begchunk,endchunk, lfrac, found, gridname='physgrid')
       if (.not.found) then
          write(iulog,*) 'LFRAC, variable lfrac not found in input file ',lfracfile
        call endrun('LFRAC :: Error, variable lfrac not found in input file')
      end if
    
    
      call pio_closefile(ncidlfrac)
      end subroutine
      !------------------------------------------
      !END LFRAC-READ
